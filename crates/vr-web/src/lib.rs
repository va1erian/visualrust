...
//! closure, and `Handler` is a trait rather than a `Fn` type. A Dyon binding can
//! therefore keep the router as-is and replace the registry with a map from name
//! to a Dyon function: a worker sends the [`Request`] to the single owner thread
//! and waits for the [`Response`]. That matches the win32ui `Msg` model and
//! keeps Dyon single-threaded. The `server_*` natives belong to #86 and are not
